// src/app/api/behavioral-analysis/route.ts
...
export async function POST(request: NextRequest) {
  ...
  const WEBHOOK_URL = process.env.N8N_BEHAVIORAL_ANALYSIS_WEBHOOK_URL;
  ...
  if (!WEBHOOK_URL || !AUTH_HEADER_NAME || !AUTH_HEADER_VALUE) {
    console.error("API Route Error: Missing n8n webhook environment variables for Behavioral Question Analyzer.");
  ...
  try {
    ...
    try {
        ...
        console.error("API Route Error: Failed to parse n8n response as JSON for Behavioral Question Analyzer:", parseError);
        return NextResponse.json(
          { message: "Invalid response format from n8n backend for Behavioral Question Analyzer." },
          { status: 500 }
        );
    }
    ...
    console.error("API Route Error: Failed to communicate with n8n webhook for Behavioral Question Analyzer:", error);
    return NextResponse.json(
      { message: "Internal server error during n8n communication for Behavioral Question Analyzer." },
      { status: 500 }
    );
  }
} 