// src/app/api/salary-negotiator/route.ts
...
export async function POST(request: NextRequest) {
  ...
  const WEBHOOK_URL = process.env.N8N_SALARY_NEGOTIATOR_WEBHOOK_URL;
  ...
  if (!WEBHOOK_URL || !AUTH_HEADER_NAME || !AUTH_HEADER_VALUE) {
    console.error("API Route Error: Missing n8n webhook environment variables for AI Salary Negotiator Assistant.");
  ...
  try {
    ...
    try {
        ...
        console.error("API Route Error: Failed to parse n8n response as JSON for AI Salary Negotiator Assistant:", parseError);
        return NextResponse.json(
          { message: "Invalid response format from n8n backend for AI Salary Negotiator Assistant." },
          { status: 500 }
        );
    }
    ...
    console.error("API Route Error: Failed to communicate with n8n webhook for AI Salary Negotiator Assistant:", error);
    return NextResponse.json(
