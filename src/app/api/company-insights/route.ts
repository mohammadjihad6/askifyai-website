...
export async function POST(request: NextRequest) {
  ...
  const WEBHOOK_URL = process.env.N8N_COMPANY_INSIGHTS_WEBHOOK_URL;
  ...
  if (!WEBHOOK_URL || !AUTH_HEADER_NAME || !AUTH_HEADER_VALUE) {
    console.error("API Route Error: Missing n8n webhook environment variables for AI Company Insights.");
  ...
  // هذه الصفحة ترسل JSON (companyName, targetJobTitle, etc.)
  ...
  try {
    ...
    try {
        ...
        console.error("API Route Error: Failed to parse n8n response as JSON for AI Company Insights:", parseError);
        return NextResponse.json(
          { message: "Invalid response format from n8n backend for AI Company Insights." },
          { status: 500 }
        );
    }
    ...
    console.error("API Route Error: Failed to communicate with n8n webhook for AI Company Insights:", error);
    return NextResponse.json(
