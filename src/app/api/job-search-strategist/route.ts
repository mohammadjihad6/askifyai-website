// src/app/api/job-search-strategist/route.ts
...
export async function POST(request: NextRequest) {
  ...
  const WEBHOOK_URL = process.env.N8N_JOB_SEARCH_STRATEGIST_WEBHOOK_URL;
  ...
  if (!WEBHOOK_URL || !AUTH_HEADER_NAME || !AUTH_HEADER_VALUE) {
    console.error("API Route Error: Missing n8n webhook environment variables for AI Job Search Strategist.");
  ...
  try {
    ...
    try {
        ...
        console.error("API Route Error: Failed to parse n8n response as JSON for AI Job Search Strategist:", parseError);
        return NextResponse.json(
          { message: "Invalid response format from n8n backend for AI Job Search Strategist." },
          { status: 500 }
        );
    }
    ...
    console.error("API Route Error: Failed to communicate with n8n webhook for AI Job Search Strategist:", error);
    return NextResponse.json(
